...
def train(args):
    ...
    path_best_model = cfg_train['path_saved_model']
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        if curr_map > best_map:
            torch.save(checkpoint_dict, path_best_model)
            best_map = curr_map
            best_ep = epoch+1
...
#             break
    f_log.write(f"Training completed! Best model saved at epoch: {best_ep}")
    f_log.close()
    print(f"Training completed! Best model saved at epoch: {best_ep}")
